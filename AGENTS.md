...
| `LumaAutoHideSettings.swift` | ~60 | `LumaAutoHideInterval` enum — defines the user-configurable auto-hide timeout options (15s, 1m, 3m, 5m, 15m, 30m, 1h, Never). Raw value is seconds; `0` means Never. Owns UserDefaults keys and `registerDefaults()`. Shared between `CompanionManager` and `SettingsPanelView`. |
| `LumaMouseWiggleDetector.swift` | ~150 | Detects rapid side-to-side mouse shake (like Apple's "shake to find cursor"). Polls `NSEvent.mouseLocation` at 30 fps, tracks X-axis direction reversals over a 400 ms window. Fires `onWiggleDetected` when ≥3 reversals occur at ≥500 pts/sec average speed. 2-second cooldown. Used by `CompanionManager` to wake Luma or reset idle timer. |
